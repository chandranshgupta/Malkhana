...
#[tauri::command]
pub fn get_archive_matrix(state: State<'_, DbState>) -> Result<Vec<ArchiveSlot>, AppError> {
    ...
    let slots = repository::get_all_archive_slots(&conn)?;
    
    Ok(slots)
}

#[tauri::command]
pub fn search_archive(query: String, state: State<'_, DbState>) -> Result<Vec<String>, AppError> {
    let guard = state.0.lock().map_err(|e| AppError::Lock(format!("Database lock failed: {}", e)))?;
    let conn = guard.as_ref().ok_or_else(|| AppError::Vault("VAULT_LOCKED".to_string()))?;
